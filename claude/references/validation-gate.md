...
- Validate the **`-o` output file** (or substantive stdout). stderr progress lines alone are not a contribution.
- Reject if the file is missing, empty, or contains only Codex status/metadata without addressing the brief.
